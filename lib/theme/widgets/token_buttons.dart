...
ButtonStyle tokenButtonStyle({
  ...
  Color resolveBg(
    ...
    if (pressed) {
      alpha = clampAlpha(alpha + dPress);
    }
    if (hovered) {
      alpha = clampAlpha(alpha + dHover);
    }
    if (focused) {
      alpha = clampAlpha(alpha + dFocus);
    }
    if (disabled) {
      alpha = clampAlpha(alpha * 0.65);
    }
  ...
  Color borderForOutline(Set<WidgetState> states) {
    ...
    if (pressed) {
      alpha = clampAlpha(alpha + 0.05);
    } else if (hovered) {
      alpha = clampAlpha(alpha + 0.03);
    } else if (focused) {
      alpha = clampAlpha(alpha + 0.02);
    }
    if (disabled) {
      alpha *= 0.6;
    }
