                ...
                onPressed: () {
                  ...
                  if (isMobile) {
                    Navigator.of(context).push(
                      MaterialPageRoute(
                          builder: (_) => const InviteGuildMemberScreen()),
                    );
                  } else {
                    final controller = Provider.of<MainContentController>(
                        ...
                        listen: false);
                    controller
                        .setCustomContent(const InviteGuildMemberScreen());
                  }
                },
              ),
            ),
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<String>(
      onSelected: (value) async {
        if (value == 'leave') {
          final confirmMessenger = ScaffoldMessenger.of(context);
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<String>(
      onSelected: (value) async {
        ...
        try {
          final callable =
          FirebaseFunctions.instance.httpsCallable('updateGuildRole');
          await callable.call({
            'targetUserId': userId,
            'newRole': newRole,
          });
          ...
          messenger.showSnackBar(
            SnackBar(
                content: Text(
                    newRole == null ? "Member kicked." : "Role updated.")),
          );
        } catch (e) {
          messenger.showSnackBar(
            SnackBar(content: Text("Error: $e")),
      ...
      itemBuilder: (context) {
        return [
          if (currentRole == 'member')
            const PopupMenuItem(
                value: 'promote', child: Text('Promote to Officer')),
